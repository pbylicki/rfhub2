...
class CollectionsApiTest(BaseApiEndpointTest):
    ...
    def test_create_new_collection(self):
        response = self.auth_client.post("api/v1/collections/", json=self.COLLECTION_TO_CREATE)
    ...
    def test_should_not_create_new_collection_with_wrong_credentials(self):
        ...
        response = self.client.post("api/v1/collections/", json=self.COLLECTION_TO_CREATE, auth=credentials)
        self.assertEqual(response.status_code, 401)

    def test_update_existing_collection(self):
        response = self.auth_client.put(f"api/v1/collections/{self.COLLECTION_3['id']}/", json=self.COLLECTION_TO_UPDATE)
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_404_when_updating_nonexistent_collection(self):
        response = self.auth_client.put("api/v1/collections/999/", json=self.COLLECTION_TO_UPDATE)
        self.assertEqual(response.status_code, 404)
