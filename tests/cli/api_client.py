...
class ApiClientTests(unittest.TestCase):
    ...
    def test_get_collections(self):
        with responses.RequestsMock() as rsps:
            rsps.add(responses.GET, f'{self.app_url}/api/v1/collections', body=json.dumps(COLLECTION),
    ...
    def test_add_collection(self):
        with responses.RequestsMock() as rsps:
            rsps.add(responses.POST, f'{self.app_url}/api/v1/collections/', body=json.dumps(COLLECTION[0]),
    ...
    def test_delete_collection(self):
        with responses.RequestsMock() as rsps:
            rsps.add(responses.DELETE, f'{self.app_url}/api/v1/collections/1/',
                     status=204, adding_headers={"accept": "application/json"})
            response = self.client.delete_collection(1)
            self.assertEqual(response.status_code, 204)

    def test_add_keyword(self):
        with responses.RequestsMock() as rsps:
            rsps.add(responses.POST, f'{self.app_url}/api/v1/keywords/', body=json.dumps(KEYWORD),
                     status=201, adding_headers={"Content-Type": "application/json", "accept": "application/json"})
