...
class KeywordRepositoryTest(BaseRepositoryTest):
    ...
    def test_should_get_all_keywords(self) -> None:
        ...
        self.assertEqual(result, self.sorted_model_keywords)

    def test_should_filter_keywords_by_name_and_doc(self) -> None:
        test_data = [
            ("Environ", [self.model_keywords[2], self.model_keywords[0]]),
            ("login_key", self.model_keywords[1:2]),
            ("Teardown", [self.model_keywords[2], self.model_keywords[0]]),
    ...
    def test_should_filter_keywords_by_name_only(self) -> None:
        test_data = [
            ("Environ", []),
            ("login_key", self.model_keywords[1:2]),
            ("Teardown", self.model_keywords[2:]),
            ("", self.sorted_model_keywords),
        ]
        for pattern, expected in test_data:
            with self.subTest(pattern=pattern, expected=expected):
    ...
    def test_should_get_all_keywords_with_collection_name(self) -> None:
        test_data = [
            ("third", []),
            ("collec", self.sorted_model_keywords),
            ("", self.sorted_model_keywords),
        ]
    ...
    def test_should_get_all_keywords_matching_pattern_and_collection_name(self) -> None:
        test_data = [
            ("Login", "collect", [self.model_keywords[1], self.app_keyword.to_model()]),
            ("Login", "second", [self.app_keyword.to_model()]),
            ("application", "Third", []),
